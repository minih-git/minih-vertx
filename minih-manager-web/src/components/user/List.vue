        ...
        <el-table-column align="center" label="角色" prop="roleInfos">
          <template #default="scope">
            <el-popover placement="right" :width="250" trigger="hover">
              <template #reference>
                <el-tag effect="plain">
                  <el-icon><TrophyBase /></el-icon>角色列表
                </el-tag>
              </template>
              <el-table :data="scope.row.roleInfos">
                <el-table-column align="center" width="150" property="name" label="角色名字" />
                <el-table-column align="center" width="80" property="roleId" label="角色编号" />
              </el-table>
            </el-popover>
...
import {ref} from "vue";
import {Timer, Iphone, Edit, Remove, Promotion, Check,TrophyBase} from "@element-plus/icons-vue";
import {UserInfo} from "../../store/module/user";
import {userList, kickOut as kickOutApi, lock as lockApi, unlock as unlockApi} from "../../api";
...
const queryUserList = () => {
  ...
  try {
    userList("").then(async it => {
      const roles: RoleInfo[] = await store.dispatch("role/setRoleCacheList")
      it.data.map(it => {
        it.roleInfos = roles.filter(it1 => it.role.includes(it1.roleId))
      })
