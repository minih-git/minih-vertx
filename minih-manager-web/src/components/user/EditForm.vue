...
import {addUser, checkMobile, editUser} from "../../api";
import {MinihError} from "../../utils";
import {anyIsEqual, idCheck, passwordCheck, userNameCheck} from "../../utils";
...
let formData = ref<UserInfo>(JSON.parse(JSON.stringify(props.editInfo)) || {
  username: "hubin",
  name: "胡斌",
  avatar: "",
  state: 1,
...
const queryRole = async () => {
  const roles = await store.dispatch("role/setRoleCacheList")
