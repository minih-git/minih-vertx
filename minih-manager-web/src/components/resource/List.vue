...
<template>
  <el-container class="user-list">
    <div class="operation">
      <el-button type="primary" @click="editInfo={type:'01'};drawer=true;drawerType=1">新增顶级菜单
      </el-button>
    </div>
    <div class="table">
          ...
          <template #default="scope">
            ...
            >
              <el-button :icon="Edit" circle size="small" type="primary"
                         @click="editInfo=scope.row;drawer=true;drawerType=4"/>
            </el-tooltip>
            <el-tooltip
                content="增加子菜单"
                placement="top-start"
            ...
            >
              <el-button :disabled="scope.row.type === '03'" :icon="AddLocation" circle size="small"
                         type="warning"
                         @click="editInfo={parentId:scope.row.id,type:(scope.row.type === '01'?'02':'00')};drawer=true;drawerType=(scope.row.type === '01'?2:3)"/>
            </el-tooltip>
          </template>
        </el-table-column>
      </el-table>
  ...
  <el-drawer
      ...
      direction="rtl"
  >
    <edit-form :edit-info="editInfo" :type="drawerType" @close="drawer = false"
...
const editInfo = ref<Partial<ResourceInfo>>({})
